...
const getLabelClassName = (styleType) => {
    switch (styleType) {
        case 'missed':
            return 'kanban-label--missed';
        case 'success':
            return 'kanban-label--success';
        default:
            return '';
    }
}
...
const BoardCard = (elem) => {
    ...
    return (
        elem?.deals?.map((deal, index) => (
            <a href={deal?.href} className='board_card' style={{ background: deal?.color }} key={`${deal?.id}-${index}`} id={`${deal?.id}-${index}`} draggable="true" onClick={(e) => e.preventDefault()}>
                ...
                {deal?.more?.map((el, index) => (
                    <div className='card_responsible' key={`${el?.value}-${index}`}>
                        <p>{el?.value}</p>
                        <span className={`kanban-label ${getLabelClassName(el.styleType)}`}>
                            {el?.label}
                        </span>
                    </div>
                ))}
